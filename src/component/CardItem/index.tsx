...
import { deleteCard, updateQuery, detailsWeather } from '../../actions/actionCreator';
...
const CardItem = ({ id, img, maxTemp, minTemp, city, humidity, isUpdating, isUpdate }: ICardItem) => {
	...
	const goingToDetails = () => dispatch(detailsWeather(id));
	...
	return (
		<Card className="card-item" sx={{ maxWidth: 345 }}>
			{isUpdating ? (
				...
				<Link onClick={goingToDetails} to={`/weather-details/${city}`}>
					<CardActionArea>
						...
						<CardContent>
							...
							<div className="card-info">
								...
								<Typography variant="body2">
									<ArrowDownwardIcon /> {Math.round(minTemp)}
								</Typography>
								<Typography variant="body2">
									<ArrowUpwardIcon /> {Math.round(maxTemp)}
								</Typography>
							</div>
